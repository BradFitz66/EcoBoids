    ...
    public class IntervalHeap<T>
    {
        ...
        public void Insert(double key, T value)
        {
            ...
            if (Size >= Capacity)
            {
                ...
                Array.Copy(tData, newData, tData.Count);
                tData = newData;

                // Expand the key array.
                var newKeys = new double[Capacity];
                Array.Copy(tKeys, newKeys, tKeys.Count);
                tKeys = newKeys;
            }
        ...
        private void SiftUpMin(int iChild)
        {
            ...
            for (int iParent = (iChild/2-1)&~1; 
                ...
                iC = iParent, iParent = (iChild/2-1)&~1)
            {
                Swap(iChild, iParent);
            }
        }
        ...
        private void SiftUpMax(int iChild)
        {
            ...
            for (int iParent = (iC/2-1)|1; 
                iParent >= 0 && tKeys[iC] > tKeys[iParent]; 
                iC = iParent, iParent = (iC/2-1)|1)
            {
                Swap(iC, iParent);
            }
        }
        ...
        private void SiftDownMin(int iParent)
        {
            ...
            for (int iChild = iP * 2 + 2; iChild < Size; iP = iChild, iChild = iP * 2 + 2)
            {
