...
namespace KDTree
{
    ...
    public class NearestNeighbour<T> : IEnumerator<T>
    {
        /// <summary>The point from which are searching in n-dimensional space.</summary>
        private double[] tSearchPoint ~ delete _; 
        ...
        public this(KDNode<T> pRoot, double[] tSearchPoint, DistanceFunctions kDistance, int iMaxPoints, double fThreshold)
        {

            // Store the search point.
            this.tSearchPoint = new double[tSearchPoint.Count];
            Array.Copy(tSearchPoint, this.tSearchPoint, tSearchPoint.Count);
